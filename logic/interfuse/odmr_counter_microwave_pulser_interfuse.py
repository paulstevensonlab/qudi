...
class ODMRCounterMicrowavePulserInterfuse(GenericLogic, ODMRCounterInterface,
                                    MicrowaveInterface):
    """
    ...
    def on_activate(self):
        ...
        self._pulse_device = self.pulsegenerator()
        pass
    ...
    def count_odmr_ref(self, length = 100):
        """ Sweeps the microwave and returns the counts on that sweep.
        ...
        self.sig_counts = np.zeros((len(self.get_odmr_channels()), length))
        self.ref_counts = np.zeros((len(self.get_odmr_channels()), length))
        # self.trigger()
        for i in range(length):
            self.trigger()
            # self._pulse_device.pulse_streamer.constant(([0, 1, 2], 0, 0))
            # sleep(0.05)
            self.sig_counts[:, i] = self._sc_device.get_counter(samples=1)[0]
            # turn off microwave
            # self._pulse_device.set_continuous_out([1, 1, 0, 0, 0, 0, 0, 0])
            # self.ref_counts[:, i] = self._sc_device.get_counter(samples=1)[0]
            self.ref_counts = 1.0

        self.trigger()
        # self._pulse_device.set_continuous_out([1, 1, 1, 0, 0, 0, 0, 0])
        # turn on microwave
        counts = self.sig_counts/self.ref_counts
        return False, counts
